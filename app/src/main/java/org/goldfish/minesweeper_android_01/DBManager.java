...
public class DBManager extends SQLiteOpenHelper {
	...
	public static void getInstance(@Nullable Context context
		, String dbPath) {
	...
	public List<MineSweeperGameInfo> onRecordsGet() {
		...
		try(var cursor = db.rawQuery(selectSQL, null)) {
			while (cursor.moveToNext()) {
				...
				if (difficultyDescriptionIndex == -1)
					throw new NullPointerException(
						"difficulty_description column not" +
							" found");
				...
				if (endTimeIndex == -1)
					throw new NullPointerException(
						"end_time column not found");
				long endTime =
					cursor.getLong(endTimeIndex);
				Result result =
					new Result(difficultyDescription,
						mines,  width,height, endTime);
				records.add(result);
			}
		}
		return records;
